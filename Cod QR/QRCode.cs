...
public partial class QRCode {
    ...
    public QRCode(int[][] mat) {
        code = mat;
        if ((code.Length - 17) % 4 != 0) throw new Exception("Size is invalid!");

        version = (code.Length - 17) / 4;
        if (version < 1 || version > 40) throw new Exception("Version is invalid!");

        int mask1 = 0, mask2 = 0;
        for (int i = 0; i <= 5; i++) mask1 |= code[8][i] << (14 - i);
        for (int i = 6; i <= 7; i++) mask1 |= code[8][i + 1] << (14 - i);
        for (int i = 0; i <= 5; i++) mask1 |= code[i][8] << (i);
        mask1 |= code[7][8] << (14 - 8);
    ...
    int[] GetAlignmentCoords() {
        ...
        result[0] = 6;
        if(num == 1) {
            for(int i = 0; i < num; i++) result[i] = version * 4 + 16 - result[i];
            return result;
        }
    ...
    void ApplyMask(int mask) {
        ...
        for(int j = 6; j <= 7; j++) code[8][j + 1] = (maskBits >> (14 - j)) & 1;
        code[7][8] = (maskBits >> (14 - 8)) & 1;
        for(int i = 0; i <= 5; i++) code[i][8] = (maskBits >> (i)) & 1;
        for(int i = 0; i <= 6; i++) code[code.Length - i - 1][8] = (maskBits >> (14 - i)) & 1;
        for(int j = 0; j <= 7; j++) code[8][code.Length - 1 - j] = (maskBits >> j) & 1;
        maskUsed = mask;
    }
}